...
def add_arguments(master):
    master.parser = argparse.ArgumentParser(
    ...
    parser_list = subparsers.add_parser(
        ...
        description="List issues for the repo"
    )

    parser_list_state = parser_list.add_mutually_exclusive_group()
    parser_list_state.add_argument(
        "-s", "--state",
        type=str,
        choices=["open", "closed", "all"],
        metavar="STATE",
        default="open",
        help="show issues of this state"
    )
    parser_list_state.add_argument(
        "--closed",
        dest="state",
    ...
    parser_list_state.add_argument(
        ...
        action="store_const",
        const="all",
        help="show both closed and open issues"
    )

    parser_list.add_argument(
        "-m", "--milestone",
        type=int,
        help="show issues with this milestone ID"
    )
    parser_list.add_argument(
        "-l", "--labels",
        type=str,
        help="show issues with these labels (comma-seperated list)"
    )

    parser_list_labels = parser_list.add_mutually_exclusive_group()
    parser_list_labels.add_argument(
        "-a", "--assignee",
        type=str,
        help="show issues assigned to this user"
    )
    ...
    parser_list_labels.add_argument(
        ...
        action="store_const",
        const="none",
        help="show issues assigned to noone"
    )
    
    parser_list.add_argument(
        "-c", "--creator",
        type=str,
        help="show issues created by this user"
    )
    
    parser_list_type = parser_list.add_mutually_exclusive_group()
    parser_list_type.add_argument(
        "-t", "--type",
        type=str,
        help="show issues of this type"
    )
    parser_list_type.add_argument(
        "--issues",
        dest="type",
    ...
    parser_list_lprint.add_argument(
        "--nolabels",
        action="store_true",
        help="don't print labels"
    )
    parser_list_lprint.add_argument(
        "--shortlabels",
        action="store_true",
        help="print a short version of labels"
    )
    
    parser_list.add_argument(
        "--nocomments",
        action="store_true",
        help="don't print comment count"
    )
    ...
    parser_assign = subparsers.add_parser(
        "assign",
        description="(Re)Assign an issue"
    )
    ...
    parser_assign_assignee.add_argument(
        "--none",
        dest="assignee",
        action="store_const",
        const=""
    ...
    # MILESTONE ARGUMENTS
    parser_milestone = subparsers.add_parser(
        "milestone",
        description="List milestones for the repo"
    )

    parser_milestone_state = parser_milestone.add_mutually_exclusive_group()
    parser_milestone_state.add_argument(
        "-s", "--state",
        type=str,
        choices=["open", "closed", "all"],
        metavar="STATE",
    ...
    parser_label = subparsers.add_parser(
        "label",
        description="List labels for the repo"
    )
